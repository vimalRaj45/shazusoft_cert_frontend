...
export default function BulkIssue() {
  ...
  useEffect(() => {
    if (selectedTemplateId) {
      api.get(`/templates/${selectedTemplateId}`).then((res) => {
        ...
        // If there's already parsed data, re-evaluate mapping with new template fields
  ...
  // Generate blank downloadable CSV template with exact matching columns (excluding autogenerated certificate ID & QR)
  const downloadTemplateCSV = () => {
    const headers = ['Recipient Name', 'Recipient Email'];
    const customFields = templateFields.filter((f) => !isSystemField(f.field_key, f.is_qr, f.label));
  ...
  // Load sample 5 rows tailored dynamically to current template custom fields
  const loadSampleCSV = () => {
    const customFields = templateFields.filter((f) => !isSystemField(f.field_key, f.is_qr, f.label));
  ...
  const processParsedData = (rows, headers, fieldsToMap = templateFields) => {
    ...
    // Auto-match headers to field keys (excluding autogenerated certificate ID and system fields)
    const customFields = fieldsToMap.filter((f) => !isSystemField(f.field_key, f.is_qr, f.label));
    const autoMap = {};
    headers.forEach((h) => {
      const norm = h.toLowerCase().replace(/[^a-z0-9]/g, '');
      if (norm.includes('certid') || norm.includes('certificateid') || norm.includes('uniquecode') || norm.includes('qrcode')) {
        autoMap[h] = 'ignore';
      } else if (norm.includes('name') || norm === 'recipient') {
        autoMap[h] = 'recipient_name';
      } else if (norm.includes('email') || norm.includes('mail')) {
        autoMap[h] = 'recipient_email';
      } else {
        // Match against template custom fields
            ...
            <div>
              ...
              <div className="surface-50 border-round-lg p-3 border-1 border-200">
                ...
                <div className="flex flex-wrap gap-1.5 align-items-center">
                  ...
                  {templateFields
                    .filter((f) => !isSystemField(f.field_key, f.is_qr, f.label))
                    .map((f) => (
                      <Tag
